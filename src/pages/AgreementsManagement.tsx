			...
			<div className="space-y-6">
				...
				<div className="bg-blue-50 rounded p-4 border border-blue-200">
					<div className="flex items-center justify-between">
						<div>
							<code className="text-lg font-bold text-blue-700">
					...
					<div className="grid grid-cols-2 gap-4">
						<div className="p-3 bg-gray-50 rounded">
							...
							<p className="text-sm font-semibold text-gray-900">
								{safeFormatDate(agreement.validFrom)}
							</p>
						</div>
						<div className="p-3 bg-gray-50 rounded">
							<label className="text-xs font-medium text-gray-500 flex items-center gap-1 mb-1">
							...
							<p className="text-sm font-semibold text-gray-900">
								{safeFormatDate(agreement.validTo)}
							</p>
						</div>
						<div className="p-3 bg-gray-50 rounded">
							<label className="text-xs font-medium text-gray-500 flex items-center gap-1 mb-1">
					...
					<div className="grid grid-cols-2 gap-4">
						<div className="p-3 bg-gray-50 rounded">
							...
							<p className="mt-1 text-sm font-semibold text-gray-900">
								{agreement.accountNumber || agreement.agreementRef}
							</p>
						</div>
						<div className="p-3 bg-gray-50 rounded">
							<label className="text-xs font-medium text-gray-500">
						...
						<div className="p-3 bg-gray-50 rounded">
							...
							<p className="mt-1 text-sm font-semibold text-gray-900 break-all">
								{agreement.contactEmail || "Not set"}
							</p>
						</div>
					</div>
				</div>
...
export default function AgreementsManagement() {
	...
	const [confirmProceed, setConfirmProceed] = useState<null | {
		duplicate: boolean;
		message: string;
		payload: any;
	}>(null);
	...
	const activateMutation = useMutation({
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.message || "Failed to activate agreement",
			);
		},
	});
	...
	const suspendMutation = useMutation({
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.message || "Failed to suspend agreement",
			);
		},
	});
	...
	const expireMutation = useMutation({
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.message || "Failed to expire agreement",
			);
		},
	});
		...
		try {
			...
			if (dup?.duplicate) {
				setConfirmProceed({
					duplicate: true,
					message:
						"This agreement_ref already exists for this source/agent. Do you want to proceed?",
					payload: form,
				});
			} else {
	...
	return (
		<div className="space-y-6">
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-gray-600 mt-1">
								Search and filter your agreements
							</p>
						</div>
						<Button
							variant="secondary"
				...
				<div className="space-y-6">
					<div className="bg-blue-50 rounded p-4 border border-blue-200">
						<p className="text-sm text-gray-600 flex items-center gap-2">
							...
							Fill in all fields to create a new agreement between an agent and
							source.
						</p>
					</div>
			...
			<Modal
				...
				title="Duplicate Agreement Detected"
			>
						...
						<div className="flex items-start gap-3">
							...
							<div>
								<p className="text-sm font-semibold text-gray-900 mb-1">
									Warning: Duplicate Agreement
								</p>
								<p className="text-sm text-gray-700">
									This agreement reference already exists for this source/agent
									combination. Creating a duplicate may cause conflicts. Do you
